...
sentinel_lines_processed_total = Counter(
    "sentinel_lines_processed_total", "Total log lines processed"
)
sentinel_alerts_total = Counter(
    "sentinel_alerts_total", "Total alerts generated", ["category"]
)
sentinel_alerts_by_source_ip_total = Counter(
    "sentinel_alerts_by_source_ip_total", "Alerts by source IP", ["source_ip"]
)
sentinel_alerts_by_path_total = Counter(
    "sentinel_alerts_by_path_total", "Alerts by path", ["path"]
)
...
class LogSentinel:
    ...
    def __init__(self, config_path="config.yaml"):
        ...
        self.whitelist_ips = {
            ip.strip()
            for ip in str(self.config.get("whitelist_ips", "")).split(",")
            if ip.strip()
        }
    ...
    def load_config(self, config_path):
        ...
        try:
            ...
            print(
                f"{Colors.RED}Error: Config file not found at {config_path}{Colors.RESET}"
            )
            sys.exit(1)
    ...
    def validate_config(self, loaded_config):
        ...
        for field in integer_fields:
            try:
                ...
                print(
                    f"{Colors.YELLOW}Warning: Invalid value for {field}, using default {self.DEFAULT_CONFIG[field]}{Colors.RESET}"
                )
        ...
        if config["behavior_baseline_min_requests"] < 5:
            config["behavior_baseline_min_requests"] = self.DEFAULT_CONFIG[
                "behavior_baseline_min_requests"
            ]
        if config["behavior_new_endpoint_min_history"] < 5:
            config["behavior_new_endpoint_min_history"] = self.DEFAULT_CONFIG[
                "behavior_new_endpoint_min_history"
            ]
        if config["behavior_new_ua_min_history"] < 5:
            config["behavior_new_ua_min_history"] = self.DEFAULT_CONFIG[
                "behavior_new_ua_min_history"
            ]
        if config["behavior_rate_spike_multiplier"] < 1.2:
            config["behavior_rate_spike_multiplier"] = self.DEFAULT_CONFIG[
                "behavior_rate_spike_multiplier"
            ]
                ...
                with open(filepath, "r") as f:
                    for line in f:
                        ...
                        if line and not line.startswith("#"):
                            try:
                                patterns[attack_type].append(
                                    re.compile(line, re.IGNORECASE)
                                )
                            except re.error:
                                print(
                                    f"{Colors.YELLOW}Warning: Invalid regex in {filepath}: {line}{Colors.RESET}"
                                )
            except FileNotFoundError:
                print(
                    f"{Colors.YELLOW}Warning: Pattern file not found: {filepath}{Colors.RESET}"
                )
    ...
    def score_alert(
        ...
        if len(decoded_path) > 120:
            score += 5
        if any(
            token in decoded_path.lower()
            for token in ["union", "<script", "../", ";", "|", "$("]
        ):
            score += 5
    ...
    def detect_attacks(self, log_entry):
        ...
        if previous_baseline == 0:
            ...
            self.ip_baseline_rate[ip] = (0.9 * previous_baseline) + (
                0.1 * float(recent_requests)
            )
        ...
        for rule_key, alert_type in attack_map.items():
            if self.config.get(f"enable_{rule_key}_detection", True):
                ...
                matched = self.check_patterns(
                    text_to_check, self.patterns.get(rule_key, [])
                )
                if matched:
                    score = self.score_alert(
                        alert_type, path=normalized_path, useragent=useragent
                    )
                    alert = {
                        "type": alert_type,
                        ...
                        "reasons": self.build_reasons(
                            matched_pattern=matched, decoded=is_decoded
                        ),
                        "ip": ip,
                    }
                    if rule_key == "scanner":
    ...
    def _check_behavioral_anomalies(
        ...
        path_history_threshold = self.config.get(
            "behavior_new_endpoint_min_history", 15
        )
        ...
        # New Endpoint
        if (
            history_count >= path_history_threshold
            and path not in self.ip_known_paths[ip]
        ):
            score = self.score_alert(
                "Behavioral Anomaly",
                path=path,
            ...
            alerts.append(
                {
                    ...
                    "score": score,
                    "reasons": self.build_reasons(
                        notes=["new-endpoint-for-ip", f"ip-history:{history_count}"]
                    ),
                    "ip": ip,
                    "path": path,
                }
        ...
        if (
            useragent != "-"
            and history_count >= ua_history_threshold
            and useragent not in self.ip_known_useragents[ip]
        ):
            score = self.score_alert(
                "Behavioral Anomaly",
                path=path,
            ...
            alerts.append(
                {
                    ...
                    "score": score,
                    "reasons": self.build_reasons(
                        notes=["new-useragent-for-ip", f"ip-history:{history_count}"]
                    ),
                    "ip": ip,
                    "useragent": useragent,
                    "path": path,
                ...
                alerts.append(
                    {
                        ...
                        "reasons": self.build_reasons(
                            recent_requests=recent_requests,
                            notes=[
                                f"baseline-rate:{previous_baseline:.1f}",
                                f"spike-ratio:{anomaly_ratio:.2f}x",
                            ],
                        ),
                        "ip": ip,
                        "path": path,
        ...
        self.ip_known_paths[ip].add(path)
        if useragent != "-":
            self.ip_known_useragents[ip].add(useragent)
    ...
    def format_alert(self, alert):
        ...
        if "reasons" in alert:
            output += (
                f"{color}Reasons: {' | '.join(alert['reasons'][:4])}{Colors.RESET}\n"
            )
    ...
    def log_alert(self, alert):
        ...
        sentinel_alerts_total.labels(category=alert.get("type", "Unknown")).inc()
        sentinel_alerts_by_source_ip_total.labels(
            source_ip=alert.get("ip", "unknown")
        ).inc()
        if "path" in alert:
            sentinel_alerts_by_path_total.labels(path=alert["path"]).inc()
    ...
    def tail_file(self, filename):
        """Tail a file like 'tail -f', handles log rotation and waits for file creation."""
        file_dir = os.path.dirname(filename)
        if file_dir:
            os.makedirs(file_dir, exist_ok=True)

        # Wait for file to be created (useful for Docker startup synchronization)
        wait_msg_shown = False
        while not os.path.exists(filename):
            if not wait_msg_shown:
                print(f"{Colors.YELLOW}Waiting for log file to be created: {filename}...{Colors.RESET}")
                wait_msg_shown = True
            time.sleep(1)
    ...
    def run(self):
        ...
        try:
            for line in self.tail_file(self.config["log_file"]):
                sentinel_lines_processed_total.inc()
            ...
            print(
                f"{Colors.CYAN}Suppressed duplicates: {self.suppressed_duplicates}{Colors.RESET}"
            )
